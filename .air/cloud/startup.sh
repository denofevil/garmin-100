...
export CHROME_FOR_TESTING_BIN="$CHROME_BIN"
export CHROME_PATH="$BIN_DIR/google-chrome"
export PUPPETEER_EXECUTABLE_PATH="$BIN_DIR/google-chrome"
...
write_chrome_wrappers() {
  mkdir -p "$BIN_DIR"
  # A stable "google-chrome" entry point for puppeteer / chrome-devtools-mcp:
  # it pulls in the userspace libraries and adds the flags this container needs
  # (no user namespaces available, so the zygote sandbox cannot start; /dev/shm
  # is small).
...
healthcheck() {
  ...
  # via the wrapper on PATH -- the same entry point puppeteer /
  # chrome-devtools-mcp use.
  "$BIN_DIR/google-chrome" \
    --headless=new \
...
if install_chrome_libs && install_chrome; then
  write_chrome_wrappers
  install_env_file
  log "chrome available at $CHROME_BIN (on PATH as google-chrome, \$CHROME_PATH=$BIN_DIR/google-chrome)"
  log "note: chrome-devtools-mcp only looks for /opt/google/chrome/chrome, which cannot be created without root here."
  log "      run it with --executablePath=\"\$CHROME_PATH\" (or add that arg to .mcp.json) to drive the extension from this environment."
else
  CHROME_BIN=""
  chrome_env_exports > "$ENV_FILE"
  install_env_file
